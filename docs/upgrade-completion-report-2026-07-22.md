...
- WordPress本体・有効プラグイン・テーマ・翻訳の更新候補は0件になった。
...
- MW WP Form 5.1.4で本文中の`script` / `style`タグだけが除去され、内容が公開画面へ露出していた問題を解消した。
- 管理者メールと自動返信メールは2通とも生成できる。実配送は移行サーバーのDNS / SMTP出口制限で停止しており、本番公開の残課題とした。
...
WordPressの更新APIによる最終確認では、WordPress本体、プラグイン、テーマ、翻訳の更新残件はいずれも0件。
...
- 移行サーバーから`downloads.wordpress.org`のZIP取得が停止したため、公式配布元からローカル取得した11パッケージを手動反映した。
- WordPressコア、9プラグイン、Lightning親テーマの計193言語ファイルをSHA-256検証付きで更新した。
- 反映後の更新画面で「翻訳はすべて最新版です。」を確認した。
- 実行用PHPとアップロードZIPは反映直後にWeb領域から削除した。
...
- フォーム本文からインラインJavaScript / CSSを削除し、同じ動作を子テーマの`page-contact.js`と`style.css`へ移した。
- 管理者メールのReply-Toを存在しない`{your_mail}`から入力項目`{your_mail01}`へ修正した。
- 管理者メールと自動返信メールの2通が`wp_mail`まで到達することを確認した。QAでは`pre_wp_mail`で配送を止めてペイロードを確認しており、実配送成功を示すものではない。
- ローカルPostfixのキューに2通が残り、宛先MXの名前解決に失敗している。SMTPサーバーIPへの25 / 465 / 587番接続はTimeout、HTTPプロキシ経由のCONNECTは403だった。
- Firefoxプライベートウィンドウで入力、確認、戻る、入力値保持、流入元ラベル切替を確認し、公開画面に生コードがないことを確認した。送信完了操作は実配送経路がないため実施していない。
- 送信先と投稿データはQA前後で不変。フォーム設定は不正なReply-To項目だけを修正し、新しい送信先が未指定のため現行アドレスを維持している。
...
### MW WP Form And Translations

- `/logs/_migration/logs/_migration/theme-before-mw-form-20260722-154020/`
- `/logs/_migration/logs/_migration/mw-form-157-before-inline-cleanup-20260722-154020.json`
  - SHA-256: `ac15a9d499e7c58bc4d3875c4a1cb21e5f4adafdf73b2b6b89e7c7d73270d076`
- `/logs/_migration/logs/_migration/languages-before-translation-update-20260722-072950.zip`
  - SHA-256: `9cc088f0d1db0bfe8939d62dd83f1b08ba1b9db3ed1ffced1b78a53634a64681`
- `/logs/_migration/logs/_migration/translation-inventory-34d57a91.json`
- `/logs/_migration/logs/_migration/translation-install-34d57a91.json`

## Final QA

- Public crawl: top, company, business, product, production, quality, recruit, six interview pages, contact, notice list/detail, privacy; 17/17 HTTP 200, maximum 0.1179 seconds.
...
- Admin browser: Dashboard, Site Health, Updates, Plugins, MW WP Form edit pageをFirefoxプライベートウィンドウで正常表示した。翻訳反映後もUpdates、Plugins、Site Healthを再確認した。
...
- Contact: 生コード露出なし、フォーム入力、確認、戻る、値保持、流入元ラベル切替、管理者メールと自動返信メールの生成を確認した。実配送は環境制約により未確認。
...
- `wp-content/themes/lightning_for_miki/assets/js/page-contact.js`
- `wp-content/themes/lightning_for_miki/page-contact.php`
- `wp-content/themes/lightning_for_miki/style.css`
- `wp-content/themes/lightning_for_miki/style.scss`
...
- **本番公開の解除条件:** 移行／本番サーバーから利用可能なメール配送経路を用意し、管理者宛と問い合わせ者宛の実メール2通が届くことを確認する。現状は公開不可。
- メール不達はWordPressやMW WP Formの生成処理ではなく、移行サーバーのDNSとSMTP出口制限が原因。ホスティング側でDNSとSMTPを許可するか、認証済みSMTPまたはHTTPSメールAPIの接続情報が必要。
- 移行サーバーのPostfixキューには診断時の2通が残っている。配送経路を開放する前に、ホスティング管理者が対象キューを確認して不要な診断メールを削除する。
...
7. Review the two queued migration test messages and delete them before enabling outbound mail.
8. Configure the confirmed MW WP Form destination in form ID 157 and run a controlled real delivery test.
9. Confirm both the administrator notification and the submitter auto-reply arrive before opening the site.
10. Re-run Dashboard, Site Health, Updates, Plugins, REST, loopback, WP-Cron, 17-page crawl, contact form and PC/mobile browser QA.
11. Check hosting PHP logs and WordPress logs for Fatal / Warning / Deprecated / Notice before ending maintenance.
